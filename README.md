...
- **Autonomous Early Warning Anomaly Detection & RCA**: A scheduled background worker checks category volumes every 15 minutes. Surges exceeding **250%** (minimum 5 complaints/hour) trigger the Root Cause Analysis (RCA) agent. It employs Llama-3.3-70b to synthesize systemic outage patterns (error codes, locations, affected payment gateways) and streams live alerts directly to officer dashboards via Socket.IO.
